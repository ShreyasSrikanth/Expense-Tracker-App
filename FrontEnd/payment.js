...
async function fetchAllexpense() {
    try {
      const response = await axios.get('http://54.162.115.208:4000/expense/fetchAllexpense');
...
premiumButton.addEventListener('click', async (e) => {
    ...
    try {
        ...
        const response = await axios.get('http://54.162.115.208:4000/premium/premiummembership', {
            headers: {
                'Authorization': token
            }
        });
        ...
        const options = {
            ...
            "handler": async function (rzpResponse) {
                try {
                    await axios.post('http://54.162.115.208:4000/premium/updateTransaction', {
